...
async artistProfile(req: Request, res: Response): Promise<void | Response<any>> {
  try {
    const email = req.tokenData.email;
    const userId = Number(req.tokenData.userId)
    const userRepository = AppDataSource.getRepository(User);
    ...
    const appointments = await AppDataSource.getRepository(Appointment).findBy({
      artist_id: artistProfile.id,
    });

    for (const appointment of appointments) {
      const user = await userRepository.findOneBy({ id: appointment.user_id });
